...
var Supplier = new schema({
    ...
    supplierNum     :   { type: String, description: "编号"}
  , supplierName    :   { type: String, description: "供应商"}
  , supplierPhone   :   { type: String, description: "联系方式"}
...
function model(dbname) {
  return conn.model(dbname, "Supplier", Supplier);
}
 ...
 */
exports.add = function(code, newSupplier, callback) {

  var supplier = model(code);

  new supplier(newSupplier).save(function(err, result) {
    callback(err, result);
  });
};
 ...
 */
exports.update = function(code, supplierId, newSupplier, callback) {

  var supplier = model(code);

  supplier.findByIdAndUpdate(supplierId, newSupplier, function(err, result) {
    callback(err, result);
  });
};
 ...
 */
exports.get = function(code, supplierId, callback) {

  var supplier = model(code);

  supplier.findOne({_id: supplierId}, function(err, result) {
    callback(err, result);
  });
};
 ...
 */
exports.remove = function(code, uid, supplierId, callback) {

  var supplier = model(code);

  supplier.findByIdAndUpdate(supplierId,  {valid: 0, editat: new Date(), editby: uid}, function(err, result) {
    callback(err, result);
  });
};
...
exports.total = function(code, condition, callback) {
  ...
  supplier.count(condition).exec(function(err, count) {
    callback(err, count);
  });
};